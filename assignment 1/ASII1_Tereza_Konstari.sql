...
FROM officers o JOIN crime_officers USING (officer_id)
...
/*2. List the names of all criminals who have committed more than average number of crimes and are not listed as violent offenders.*/
SELECT c.last, c.first
FROM criminals c JOIN crimes USING (criminal_id)
WHERE v_status = 'N'
...
FROM prob_officers p JOIN sentences USING (prob_id)
...
SELECT crime_id, cr.classification, cr.date_recorded
FROM crimes cr JOIN appeals USING (crime_id)
GROUP BY crime_id, cr.classification, cr.date_recorded
HAVING COUNT(crime_id) < (SELECT COUNT(appeal_id)
                            FROM appeals)
