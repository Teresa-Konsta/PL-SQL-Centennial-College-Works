...
dd_id_track NUMBER(8) PRIMARY KEY,
dd_username VARCHAR2(15),
dd_current_date DATE,
dd_action VARCHAR2(6),
dd_idpay NUMBER(7,2)
);
...
CREATE OR REPLACE TRIGGER DD_PAYTRACK_PLEDGEPAY
AFTER INSERT OR DELETE OR UPDATE
ON dd_payment
FOR EACH ROW
DECLARE
    action dd_paytrack.dd_action%TYPE;
    idpay dd_paytrack.dd_idpay%TYPE;
BEGIN
    idpay := :NEW.idpay;
    ...
    action := 'Delete';
    ELSE
    DBMS_OUTPUT.PUT_LINE('Enter adequate values.');
    END IF;

    INSERT INTO DD_PAYTRACK (dd_id_track, dd_username, dd_current_date, dd_action, dd_idpay)
      VALUES(DD_PTRACK_SEQ.NEXTVAL, USER, TO_CHAR(SYSDATE, 'DD-MON-YY'), action, idpay);
END;

INSERT INTO dd_payment(idpay, idpledge, payamt, paydate, paymethod)
    VALUES (1465, 120, 50, SYSDATE, 'CC');
    COMMIT; 
...
CREATE OR REPLACE TRIGGER DD_PLEDGE_FIRSTPLEDGE
AFTER INSERT
ON DD_PLEDGE
DECLARE
    ...
    SELECT *
    FROM DD_PLEDGE;
        first_plg DD_PLEDGE.FIRSTPLEDGE%TYPE;
        id_donor DD_PLEDGE.IDDONOR%TYPE;
BEGIN
    /*old_id_donor := :OLD.iddonor;
    new_id_donor := :NEW.iddonor;*/
        
    IF (OLD.id_donor = NEW.id_donor) THEN
    first_plg := 'Y';
    ELSE
    first_plg := 'N';
    END IF;
...
INSERT INTO DD_PLEDGE (idpledge, iddonor, pledgedate, pledgeamt, idproj, idstatus, writeoff, paymonths, campaign)
    VALUES (113, 309, '03-AUg-2020', 500, 515, 30, null, 18, 791)
